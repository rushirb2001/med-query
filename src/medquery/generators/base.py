...
class QueryGenerator:
    ...
    def __init__(self, model: str = DEFAULT_MODEL, use_cache: bool = True, max_concurrency: int = 10):
        ...
        logger.info(f"QueryGenerator initialized: model={model}, cache={use_cache}")
    ...
    def generate_queries(
        ...
        """
        logger.info(f"Generating {count} queries for category={category}, level={level}")
        queries = []
        ...
        logger.info(f"Generated {len(queries)} queries for {category}")
        return queries
    ...
    def _generate_batch(
        ...
        logger.debug(f"Building prompts for batch of {count}")
        ...
        logger.debug(f"Calling API for batch of {count}")
        if self.use_cache:
        ...
        if hasattr(response, "usage"):
            ...
            logger.debug(f"API response: in={input_tokens}, out={output_tokens}, cached={cached}")

            # Warn if output might be truncated
            if output_tokens >= max_tokens - 100:
                logger.warning(f"Response may be truncated ({output_tokens}/{max_tokens} tokens)")
        ...
        if len(queries) == 0 and retry_count < max_retries:
            ...
            logger.warning(f"Parse failed, retrying with smaller batch ({smaller_count})")
            time.sleep(1)
...
Return JSON array now:"""
    ...
    def save_queries(self, queries: list[GeneratedQuery], filepath: str):
        ...
        logger.info(f"Saved {len(queries)} queries to {filepath}")
    ...
    async def generate_queries_async(
        ...
        Returns:
            List of generated queries
        """
        logger.info(f"Generating {count} queries async for category={category}")
        batches = (count + batch_size - 1) // batch_size
        ...
        logger.info(f"Generated {len(results)} queries async for {category}")
        return results
    ...
    async def _generate_batch_async(
        ...
        logger.debug(f"[{batch_num}/{total_batches}] Calling API for batch of {count}")
        ...
        if hasattr(response, "usage"):
            ...
            output_tokens = getattr(usage, "output_tokens", 0)
            cached = getattr(usage, "cache_read_input_tokens", 0)
            logger.debug(f"[{batch_num}/{total_batches}] Done: in={input_tokens}, out={output_tokens}, cached={cached}")
...
def load_queries(filepath: str) -> list[dict]:
    ...
    logger.debug(f"Loading queries from {filepath}")
