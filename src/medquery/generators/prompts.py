"""Master prompt for query generation with Qwen-specific formatting.

Provides the unified prompt for Qwen2 MLX models with:
...
Note: This is for DATASET GENERATION, not inference.
For inference prompts, see medquery.backends.prompts.
"""

from ..types import OUTPUT_SCHEMA_STR, EXAMPLE_OUTPUTS
...
class MasterPrompt:
    """Unified prompt for medical query classification (Qwen-specific)."""
...
{{"medical":{{"value":false,"confidence":0.95}},"intent":{{"primary":null,"secondary":null}},"entities":[],"relationships":[],"decompose":false,"sub_queries":null,"retrieval_hint":null}}"""
    ...
    def get_full_prompt(cls) -> str:
        """Get the complete prompt with all sections (Qwen format)."""
        logger.debug("Building full Qwen prompt")
...
"""
    ...
    def get_minimal_prompt(cls) -> str:
        """Get a minimal prompt for faster inference (Qwen format)."""
        logger.debug("Building minimal Qwen prompt")
...
"""
    ...
    def get_prompt_for_intent(cls, intent: str) -> str:
        ...
        logger.debug(f"Building intent-specific prompt for: {intent}")
        intent_prompts = {
