...
def validate_query(query_dict: dict) -> tuple[bool, list[str]]:
    ...
    if "expected" in query_dict:
        ...
        if "entities" in expected:
            valid_entity_types = list(ENTITY_TYPES.keys())
        ...
        if "relationships" in expected:
            valid_rel_types = list(RELATIONSHIP_TYPES.keys())
...
def deduplicate_queries(
    ...
    logger.debug(f"Deduplicated {len(queries)} queries: {len(unique)} unique, {len(duplicates)} duplicates")
...
def merge_datasets(filepaths: list[str], output_path: str):
    ...
    """
    logger.info(f"Merging {len(filepaths)} datasets")
    all_queries = []
    for fp in filepaths:
        with open(fp) as f:
            queries = json.load(f)
            all_queries.extend(queries)
            logger.debug(f"Loaded {len(queries)} from {fp}")
    ...
    logger.info(f"Saved {len(unique)} queries to {output_path}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python -m medquery.generators.validate <dataset.json>")
